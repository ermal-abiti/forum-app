<template>
    <div>
        <section class="hero is-medium">
            <div class="hero-body">
                <div class="container has-text-centered">
                    <div class="columns is-vcentered">
                        <div class="column is-6">
                            <figure class="image is-5by3">
                                <img src="../components/photos/forum.jpeg" />
                            </figure>
                        </div>
                        <div class="column is-5 is-offset-1">
                            <h1 class="title is-2">Forum App</h1>

                            <div class="content">
                                <h2 class="subtitle is-5">
                                    Forum App is an online discussion board where people can ask questions, share their experiences, and discuss topics of mutual interest. Forums are an excellent way
                                    to create social connections and a sense of community. They can also help you to cultivate an interest group about a particular subject
                                </h2>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <section class="hero is-small is-light">
            <div class="hero-body">
                <h1 class="title is-2 has-text-centered">Meet our team</h1>
                <br />
                <div class="container has-text-centered">
                    <div class="columns is-hcentred">
                        <div class="column is-2">
                            <figure class="image is-256x256">
                                <img class="is-rounded" src="../components/photos/Albion.jpeg" />
                            </figure>
                            <br />
                            <h1 class="title is-4">Albion Babaj</h1>
                            <div class="content">
                                <h3 class="subtitle is-6">
                                    Albion likes frontend . <br />
                                    
                                </h3>
                            </div>
                        </div>
                        <br />
                        <div class="column is-2">
                            <figure class="image is-256x256">
                                <img class="is-rounded" src="../components/photos/Ermal.jpeg" />
                            </figure>
                            <br />
                            <h1 class="title is-4">Ermal Abiti</h1>

                            <div class="content">
                                <h3 class="subtitle is-6">
                                    Ermal is a backend enthusiast. <br />
                                    Really likes writing restful apis.
                                </h3>
                            </div>
                        </div>

                        <div class="column is-2">
                            <figure class="image is-256x256">
                                <img class="is-rounded" src="../components/photos/Erolli.png" />
                            </figure>
                            <br />
                            <h1 class="title is-4">Eroll Uka</h1>
                            <div class="content">
                                <h3 class="subtitle is-6">
                                    Eroll is a song writer. <br />
                                    He likes fullstack.
                                </h3>
                            </div>
                        </div>

                        <div class="column is-2">
                            <figure class="image is-256x256">
                                <img class="is-rounded" src="../components/photos/Leon.jpeg" />
                            </figure>
                            <br />
                            <h1 class="title is-4">Leon Berisha</h1>
                            <div class="content">
                                <h3 class="subtitle is-6">
                                    Leon likes frontend. <br />
                                    His hobby is driving.
                                </h3>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
    </div>
</template>

<script>
export default {
    name: 'About',
};
</script>
